...
const Editor = () => {
  ...
  const checkSpelling = async () => {
    try {
      ...
      setLoading((prev) => ({ ...prev, spell: false }));
    }
  };
  ...
  const rephraseSentence = async () => {
    try {
      ...
      const results = response.data.rephrasedSentences || [];

      // If API couldn't return valid rephrase
      if (
        results.length === 1 &&
        results[0] === "No valid text returned. Try again."
      ) {
        setRephrasedSentences(["No valid text returned. Try again."]);
      } else {
        setRephrasedSentences(results);
      }

    } catch (error) {
      console.error("Error rephrasing sentence:", error);
    } finally {
      setLoading((prev) => ({ ...prev, rephrase: false }));
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-white-100 min-h-screen">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {/* ================= Left Main Editor Section ================= */}
        <div className="md:col-span-2">
          <div className="bg-pink-100 shadow-lg rounded-lg p-6 mb-8">
          ...
          {/* ================= Rephrase Section ================= */}
          {selectedSentence && (
            <div className="bg-pink-100 shadow-lg rounded-lg p-6 my-8">
          ...
          {rephrasedSentences.length > 0 && (
            <div className="bg-pink-100 shadow-lg rounded-lg p-6 my-8">
              ...
              {rephrasedSentences.map((sentence, index) => (
                <div
                  key={index}
                  className="mb-4 pb-4 border-b border-gray-200 last:border-b-0 text-center"
                >
                  <p className="mb-2">{sentence}</p>

                  {sentence === "No valid text returned. Try again." ? (
                    <Button
                      onClick={() => setRephrasedSentences([])} // hides the message box
                    >
                      Got it
                    </Button>
                  ) : (
        ...
        <div className="md:col-span-1">
          <div className="bg-pink-100 shadow-lg rounded-lg p-6 sticky top-8">
            <h3 className="text-xl font-semibold mb-4 flex items-center">
...
const Button = ({ onClick, children, icon, loading }) => (
  <button
    ...
    } text-white px-4 py-2 rounded-full font-semibold transition duration-300 flex items-center`}
  >
    {loading ? (
...
const ResultSection = ({ title, text, onAccept, icon }) => {
  ...
  if (!text || !visible) return null; //  hide component if not visible

  return (
    <div className="bg-pink-100 shadow-lg rounded-lg p-6 transition-all duration-300">
      <h3 className="text-xl font-semibold mb-4 flex items-center">
        {icon}
      ...
      <Button
        onClick={() => {
          onAccept();     // keep existing accept logic
          setVisible(false); // hide component after accepting
        }}
        icon={<FaCheck />}
      >
        Accept
